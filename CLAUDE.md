...
./tools/test.sh               # 202075 headless checks, ~200s
...
This is **consistent** with the diversion design in `docs/CITY.md`, "Guiding her to the calm", and
a session once wrongly flagged it as contradicting it. A road closure there is *"not lethal but
prevents full access"* — an absolute stop that does not kill and does not shout, which is exactly
what a silent closure is. The things that guide by being **expensive** rather than by being
impassable are ordinary catalogue events, and they already emit. Nothing about diversions asks a
closure to emit; what it asks is that closures and events be **placed to point somewhere**, which
is a scheduler decision and not a change to what a closure is.
