...
enum Kind { TREE, PLAYGROUND_FRAME, BOLLARD, STREET_TREE }
...
## This prop's own ground shape — a point for a tree, a street tree or the bollard, sized off the
## same texture fraction the shadow always used; a capsule for the swing frame
## (`_playground_frame_shape()`). Computed once `_ready()` fires, by which point `city.gd` has
## already set `kind`, `variant` and `scale_factor` on the new node (`Prop.new()` then the three
## exports, then `add_child()`), and read by `_draw()` for the shadow. Only a street tree also has
## a body — `TRUNK_RADIUS`, not this shape — since a canopy is walked under and a trunk is not.
