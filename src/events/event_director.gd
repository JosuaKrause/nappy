...
##
## **And a pursuer is sited outside the view along the heading in play, not against a flat number
## sized for one axis.** *(2026-09-07: "bikers / unleashed dogs all pop in in front of the player
## instead of starting off screen", and, of the day-3 dog specifically, "the run tutorial spawns
## inside the visible area making the headsup way too short now".)* A flat 200px sits inside the
## 320px horizontal boundary, so a dog sited while she walked east or west had no offscreen phase at
## all — it appeared already on screen. `Tuning.offscreen_boundary(heading)` asks the real question
## instead: how far to the edge of the view *this* heading actually reaches, 320px on the horizontal
## axis and 180 on the vertical rather than one number that only ever covered the narrower of the
## two.
##
...
func _crossing_ahead_of(at: Vector2, heading: Vector2,
	...
	var lead := Tuning.offscreen_boundary(heading) if def and def.pursues \
...
## outside the view along her heading — `Tuning.offscreen_boundary(heading)`, which for a row on the
## horizontal axis is 320px rather than a flat number sized for the narrower vertical one — and
## travelling back down the same line she is walking, so a rig that keeps going meets it on a
## genuine collision course rather than a near miss that depends on nobody moving.
## *(2026-09-07: "bikers / unleashed dogs all pop in in front of the player instead of starting off
## screen".)* A row sited at a flat 200px was already on screen on that axis; this is not.
##
...
func _toward_her(at: Vector2, heading: Vector2, def: EventDef) -> PackedVector2Array:
	var lead := Tuning.offscreen_boundary(heading)
