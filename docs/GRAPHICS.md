...
| M106 — Roofs, fronts and street trees | `tree_pit.svg` is a prepared, centre-anchored 32×32 ground part — see "City ground and buildings" above for the roof units, now live. `assets/buildings/storefront_{a,b,c,d}{,_awning,_shuttered}.svg` are 32×32 facade states with a shared door position; `fire_escape_{a,b}.svg` are transparent 48×64 overlays; `window_{tall,shuttered}_{dark,lit}.svg` are 32×32 overlays. Registration is detailed in the environment inventory; Prop does not yet bind `tree_pit.svg`, and the `_shuttered` storefront and `window_{tall,shuttered}` states stay unbound for M105. |
