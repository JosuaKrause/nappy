...
| Steam | `steam_pipe.svg`, `steam.svg` and `steam_b.svg`: the vent's pipe and the two frames of its cloud, three layers on one 32×48 canvas with one bottom-centre anchor, the foot of the pipe. **The pipe is not the blow's picture**: `InteriorEvents` stands it at every vent as a sprite in the building's y-sorted layer for the whole section, through `EventInstance.steam_pipe()`, so the vent is on screen blowing or not. Each blow is an instance drawing only the cloud through `_draw_simple()`, over the pipe's mouth, since it shares the pipe's y and joins the layer after it. The second frame is the plume billowing — the cloud lifted and spread, a fresh puff pushing out of the mouth under it — and the two alternate off `_idle_stepping(STEAM_BILLOW_PERIOD)`, a 0.7s cycle, for the instance's whole life, which is one blow of the vent. Nothing here is mirrored: every blow is sited facing east, and the pipe is drawn as authored. |
