...
| `m158_stair_side_{upper,lower,continue}_{e,w}.svg` | `InteriorScene._add_flight_side()` binds the reviewed 32×32 sources in the structural layer after each full flight deck and before its unchanged rails. Three complete upper/lower/continuation modules fill the deck's five-tile height: every east module shifts one tile right and down, every west module shifts one tile left and down, and no extra end role is needed. The deck retains its tread surface; these sources replace only its side reading. Neither `InteriorTileSet` nor `InteriorMap` loads them, so collision, slope redirection, landings and the route stay unchanged. The native/3× source renders and two-module shaft-background review are in `docs/evidence/m158-stair-tile-prototype-2026-09-19/`. |
