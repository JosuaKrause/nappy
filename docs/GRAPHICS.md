...
| `m158_stair_side_{upper,lower,continue}_{e,w}.svg` | Prepared 32×32 SVG-only stair-side prototype. The three roles stack vertically and a following eastward module shifts one tile east and down; westward sources mirror the geometry. These files are unbound: `InteriorTileSet`, `InteriorScene`, collision and the live stair kit do not load them. Their native/3× source renders and two-module shaft-background review are in `docs/evidence/m158-stair-tile-prototype-2026-09-19/`. |
