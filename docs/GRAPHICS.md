...
|---|---|
| M108 — Eight-direction entity graphics | `assets/events/fire_engine_front_diagonal.svg`: a southeast-facing fire engine with separate front and side planes, a roof ladder and grounded wheel axles. The 56×48 canvas has bottom-centre anchor (28, 48); southwest uses a horizontal mirror. The live fire-engine binding still selects its original side/end pair. |
