...
| Mouse | `mouse.svg`: one side-on picture, mirrored east/west like `delivery_van` rather than switching pose — see `docs/EVENTS.md`'s `alley_mouse` row. `mouse_b.svg` is its own dash's second frame — the tail curls differently rather than crossing legs it has no room to draw at this scale — alternated by `_gait_stepping()`. The `mouse_{front,back}[_diagonal].svg` family, and its own `_b` companions authored alongside it, stay unbound — see the prepared table below: `EventCatalogue._alley_mouse()`'s own docstring documents, with its reasoning, that the row is not given a second posture or a heading-selected picture. |
...
| `art/checkpoints/guard_{standing,lunging}_{front,back,side,front_diagonal,back_diagonal}.svg` (standing 22×44, anchor (11, 44); lunging 36×44, anchor (17, 44)) | The guard who moves — a heated `roadblock`'s and the masked pursuer — draws the single `guard_standing.svg`/`guard_lunging.svg` pair, and no queued item gives him a heading-selected view. |
