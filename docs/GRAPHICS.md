...
| `art/closures/barrier_post.svg` | Only around the one fenced calm area (`ParkClosure.posts()`): the post a run of barrier turns a corner on, or ends on where it turns none. 14×26, feet-anchored at the bottom centre; steel grey like the panels' own posts, but as wide as `along`'s two rails together (8px), capped and on a foot plate, and a little taller than the panels, so at a corner the broadside rails end behind its middle and the end-on column runs into it square. A street closure's line runs wall to wall and has none. |
