...
| `art/buildings/fire_escape_a.svg`/`fire_escape_b.svg` | `src/city/building.gd` bolts one to a seeded share of `RESIDENTIAL` facades tall enough for it, as an overlay over the ground floor's two bottom rows — a composition change distinct from the storefront's, since an overlay draws after the wall rather than replacing one of its textures. 48×64, anchor (24, 64) on the ground line at its column's centre, reaching eight pixels into each neighboring column. Both are black-painted iron and end at the first floor, as a real one does (`docs/playtests/PLAYTEST-124.md`, statement 10): a railed balcony across the first floor on knee brackets, in front of that floor's window, with its drop ladder stowed upright against the wall at one end. Nothing comes down to the sidewalk; below the balcony there are only its brackets and the shadow the iron casts on the wall down and to the east. `a` stows its ladder at the west end and has a potted plant on the balcony; `b` is the same escape mirrored, with nothing on it. |
...
listed above, the archived one is the 64×64 whole switchback. Exterior `fire_escape_{a,b}.svg` stays 48×64, a
first-floor balcony with no flight; that pair is unaffected by this milestone. The
