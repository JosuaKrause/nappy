...
| `art/buildings/fire_escape_a.svg`/`fire_escape_b.svg` | `src/city/building.gd` bolts one to a seeded share of `RESIDENTIAL` facades tall enough for it, as an overlay over the ground floor's two bottom rows — a composition change distinct from the storefront's, since an overlay draws after the wall rather than replacing one of its textures. 48×64, anchor (24, 64) on the ground line at its column's centre, reaching eight pixels into each neighboring column. Both are black-painted iron: a railed balcony across the first floor on knee brackets, in front of that floor's window, and a drop flight hung from one end that runs down parallel to the facade and stops short of the pavement, all casting a shadow on the wall down and to the east. `a` hangs its flight from the east end and has a potted plant on the balcony; `b` is the same escape mirrored, with nothing on it. |
| `art/props/civic_portico.svg` | `src/city/building.gd` draws one at every `CIVIC` building's entrance, centred on the facade, as the same kind of overlay a fire escape is. |
...
listed above, the archived one is the 64×64 whole switchback. Exterior `fire_escape_{a,b}.svg` stays 48×64, its flight
