...
| Ice-cream van; lorry | `ice_cream_van.svg`, `lorry.svg`: one moving picture per look. |
...
| Abduction | `unmarked_van.svg`, `unmarked_van_end.svg` (the van itself, unchanged — a vehicle row, owned by the next M108 item) and `van_victim_{view}.svg`: the victim is placed beside the van during the abduction, its view read from the direction of the short walk toward it, which is always due east or west by construction. The old unsuffixed `van_victim.svg` is unused once the named views cover it (no badge of its own). |
