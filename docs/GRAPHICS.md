...
| `art/props/door.svg`, `art/buildings/home_door_sealed.svg` | `src/city/city.gd` places the home door as a `Sprite2D` (`City._spawn_home()`), the ordinary picture until day 10's raid actually arrives at her building out of her sight (`ResistanceHappenings._maybe_raid()`, `City.seal_home_door()`): sealed from then on, for the rest of the run, every dawn and every reload included (`City._sync_home_door()`, reading the scar `City.SEALED_DOOR_SCAR` leaves). `home_door_sealed.svg` shares the ordinary door's 26×34 canvas and feet anchor (bottom centre, on the ground plane) — a regime notice pasted across the upper panel and two paper seal strips across the lock-side gap, cut there so the door still opens. |
| `art/buildings/window_boarded_sealed.svg` | `src/city/building.gd` draws this 32×32 overlay, in the windows' own registration and transparent elsewhere, over one upper-floor window cell of the one home-block building the door notch stands in front of — `Building.neighbor_window_col`, set once by `City.board_neighbor_window()` from day 11's morning on (`ResistanceHappenings.start_day()`), the third floor (wall row 3, the ground floor being row 0) nearest the door (`Building.neighbor_window_row()`) — her own floor, down the hall from her own door. That building's height is fixed at four wall rows (M185, `docs/CITY.md`, "The home"), so row 3 is always there — off the top of the screen at the normal camera on her doorstep, since she starts facing away down the street and the camera's own look-ahead leads the view with her facing, but in frame once she turns to face the building. Three planks and the same notice as the door's, drawn over whichever window picture the cell already has so the front's own lintel and sill stay. |
...
| `art/props/signal_head{,_back,_side}.svg` | `src/city/traffic_light.gd` chooses face-on, rear or edge-on traffic-light hardware by the arm's direction; code adds the lit lamp. The lenses are drawn dark in the picture, so with the power out (`TrafficSignals.powered`) the head is drawn with no lamp and is a dead light. |
...
| `art/rig/{mother,father}_carrying_{front,back,side,front_diagonal,back_diagonal}_{a,b,c}.svg` | `src/player/stroller.gd` selects the same parent's carrying family when `Stroller.carrying` is set, which `Main` sets for the whole of the escape — see "Interior (the escape scene)" below for how a run reaches it. Three distinct poses play A, C, B, C: open contact, feet together, opposite contact, feet together. Movement distance advances the loop; stopping selects C. Five authored views supply eight directions through the same west mirrors as the pushing set. The baby's cue (`baby_{zzz,fuss,cry}.svg`) draws over the bundle at the parent's own position, with no pram sprite. |
...
| `icon.png` | `project.godot` uses the root 256×256 raster as the application icon, including the window/dock icon and the web export's generated favicon (`export_presets.cfg`'s `html/export_icon=true`). |

### Interior (the escape scene)

The escape is the run's ending: a won day 14 with every task complete
(`Main._hands_over_to_the_escape()`, the same test as the good ending) reloads the scene onto it,
and `--start-escape` boots straight into it for a rig or a person.
...
| `chandelier.svg` | Hung at each hallway midpoint, clear of the lobby barricade. Unlit, with dead grey bulbs and no pool of light under it, for the same reason as the wall lamp. |
| `puddle.svg`, `basement_debris.svg`, `rat.svg` | Ground decals over `InteriorMapPlan.decals`, in the basement corridor. |

### Closures

`src/routes/closure_marker.gd` owns the street closures and `park_fence_marker.gd` draws the
connected park fence. These are separate from the event pictures with similar nouns.
...
| `art/closures/barrier_post.svg` | Prepared 14×26 broad post, bottom-center anchor. `ClosureMarker.Piece.POST` can draw it; the city uses the park's own narrower joint for park closures. Street closures have no separate end posts. |
| `art/closures/park_rail_along.svg` | The park's end-on rail, 6×22, on `street_kit`. `ParkFenceMarker` repeats its narrow top surface along the ground and keeps the near face's 5.6px height fixed. Both rails share X; their top edges stand 21px and 12.6px above the same ground line, matching `barrier_across.svg`. The upper rail occludes most of the lower in a long side run. No elevation picture is rotated. |
| `art/closures/park_post.svg` | The park's upright support and corner joint, 18×27, bottom-center anchor, on `street_kit`. A square shaft, cap, two mounting collars and broad foot show the upright structure. Side rails mount 4px inward of the pole center so the shaft remains visible, with supports spaced about 44px apart. One post joins both axes at each corner; unjoined runs end on the same post. |
| `art/closures/park_sign.svg` | The park's closed plate and its support as one 22×36 composite, bottom-center anchor, on `street_kit`. One stands at the exact midpoint of each run. The sign is drawn after the nearest rail panel that can overlap its plate, with an offset preserving the sign's ground point; later panels cannot cross its face. |
...
**The vehicles that move roll on wheels of their own.** The police car, fire engine, unmarked van,
riot van, army truck and lorry (`EventInstance.WHEELS_BY_LOOK`) each have a
`<picture>_wheels.svg` beside every one of their five views — `police_car_wheels.svg` beside
`police_car.svg`, `police_car_front_wheels.svg` beside `police_car_front.svg`, and so on — on the
same canvas and bottom-centre anchor, holding the tyres alone; the view's own picture is the body.
While the vehicle covers ground, `EventInstance._current_bob()` is `WheelBob.lift()` of it, the same
1px rise and fall a crowd car rides, in place of the taller stride bob a walking event takes;
`_draw()` and `EntityHalo` lift the drawing by it, and `_draw_eight_view()` draws the wheels and
the shadow back down by it, so only the body moves. The wheels go over the body, except the end
views of the 34×50 fire engine, vans, army truck and lorry (`WHEELS_BEHIND_THE_BODY`), whose tyres
along both flanks were always under the body's edge — at rest every split view is the same picture
it was before the split. The screen-edge badge draws `EventInstance.icon_wheels_for()` over
`icon_for()`'s side body in the same rectangle. The parked delivery and ice-cream vans never move
and keep their tyres in their one picture.
...
| Mouse | `mouse_{front,back,side,front_diagonal,back_diagonal}.svg` (`side` is the existing `mouse.svg`), its view read from the dash's own crossing heading through `EventInstance._draw_eight_view()` — the same helper every other moving family in this table uses, no `side_faces_west` override since `mouse.svg` is east-authored (`docs/evidence/svg-vehicles-2026-09-10/facings.csv`) — see `docs/EVENTS.md`'s `alley_mouse` row. `mouse_b.svg` and its own `mouse_{front,back}[_diagonal]_b.svg` siblings are the dash's own second frame — the tail curls differently rather than crossing legs it has no room to draw at this scale — alternated by `_gait_stepping()`. The old unsuffixed `mouse.svg`/`mouse_b.svg` stay live as the `side` view and, for `mouse.svg`, as `icon_for()`'s own badge silhouette. |
| Neighbor | `neighbor_{view}.svg` and `neighbor_{view}_b.svg` (`{view}` as above): the `neighbor` row, the neighbor down the hall in work clothes — the passer-by's own body (`person_{view}.svg`, path for path, 18×42, anchor (9, 42)) in steel-blue coveralls from collar to ankle, a yellow reflective band across the chest, and a dark work cap over the passer-by's hair with its brim to the front, so the figure reads as the same person from every side and as nobody else in the street. `_b` is the passer-by's own feet-passing frame, alternated by `_gait_stepping()` as it walks; `icon_for()` names `neighbor_front.svg`. The same cap is on the wanted notice's `neighbor_slot`. |
...
| Police car | `police_car_{front,back,side,front_diagonal,back_diagonal}.svg` (`side` is the existing `police_car.svg`) and each view's `_wheels.svg`, the tyres the body rides over — see the paragraph above the table: `police_patrol` is mobile and turns along its own patrol route, so `EventInstance._draw_eight_view()` reads its actual travel heading with no `side_faces_west` — its `side` picture is already east-authored — and this is the first vehicle row whose diagonal views are ordinarily reachable rather than a dead branch, since a patrol car actually turns corners. The old `police_car_end.svg`, drawn for both north and south headings alike, is superseded by the two-way `front`/`back` split and stays on disk unbound. |
| Roadblock (the impassable barrier row) | `roadblock_segment.svg` (22×30) broadside on and `roadblock_segment_vertical.svg` (20×30) end-on, picked by the spread's axis the way the roadworks barrier's two segments are (`EventInstance._roadblock_segment_texture()`), and `roadblock_end.svg` at the ends, repeated and capped by `_draw_spread()` — the same rail-with-caps construction the roadworks barrier uses — so the band reads as one continuous barrier rather than a row of blocks. The broadside segment is a poured-concrete barrier with a cream panel of red diagonal stripes; the end-on one is the same concrete and panel turned on its side for a band stacked down a north-south column: the dark plinth line down both outer edges, the lit cap down the left, the striped panel between bands of body grey. Every plane runs the full height and its stripes repeat every 15px, twice per segment, so the stacked copies meet on the same phase and read as one barrier running away from the camera. `checkpoint_block.svg` (kept from before the row was renamed) is now only the screen-edge badge's own icon. A guard is drawn beside the band from the moment it is placed for every side `EventDef.guard_sides` names, never over it: the eight-view `guard_standing_{front,back,side,front_diagonal,back_diagonal}.svg` family, each one's whole picture clear of the barrier's, facing out from it (`EventInstance._guard_post_offset()`). `[-1, 1]` (both sides) is the row's own default and what the catalogue roadblock draws; the region wall overrides it on both of its own shapes — `SealPlanner.place_hard_on()` builds its road body, between its two sidewalk ones, with no sides at all, and `RegionPlanner._alley_mouth_wall_body()` builds each of a crossing alley's two mouth bodies with the one side that faces out of the alley rather than the one that would stand inside it: see `docs/CITY.md`, "Regions and the wall". Once heated past `Tuning.HEAT_HUNTS_LEVEL` the one on her side leaves his post and `EventInstance._draw_roadblock()` keeps the barrier and the other guard drawn where they stand while he takes `guard_standing_*` (closing to his stand-off) then `guard_lunging_{view}.svg` (giving chase), each read from the guard's own travel heading through `_draw_eight_view()` — the same helper every other moving family above uses, no `side_faces_west` override since the guard's `side` picture is east-authored (`guard_lunging_side.svg`'s own comment). The unsuffixed `guard_standing.svg`/`guard_lunging.svg` stay live only beside the stationary checkpoint kit below, which never turns. This is the impassable event row, not the traversable checkpoint kit below. |
...
| Firefight | `gunman.svg`: mirrored figures form both sides; muzzle flashes are drawn circles. The `gunman_{front,back,front_diagonal,back_diagonal}.svg` views stay unbound: the two shooters face along a fixed local axis by construction (the line between them), which the side view already draws, and `gunman_side.svg` is the same picture as `gunman.svg` — there is no heading a firefight's own composite could ever turn to reach the other three. |
...
| Impact crater | `art/props/impact_crater_2x2.svg`, not `art/events/`: a 64×64 hole in the road drawn flat and **centred** on its own ground point through `EventInstance._draw_at_anchor()`, which is what makes it the one look in the catalogue that is ground rather than an object standing on ground. It carries **no drop shadow** for the same reason — a shadow under a hole reads as a mound. The `impact_crater_{1x1,3x3}.svg` sources stay unbound: `spawns_on_finish` names one row, so a second size would need a second explosion row to leave it, and three sizes of the same hole would be three looks nobody could tell apart on the screen-edge badge. |
| Masked pursuer | The eight-view `guard_standing_{front,back,side,front_diagonal,back_diagonal}.svg` family while the telegraph runs and `guard_lunging_{view}.svg` once he is coming, each read from his own travel heading through `_draw_eight_view()` — the same pair a heated `roadblock`'s guards take when they leave the post, on a row that was never a barrier. The unsuffixed `guard_lunging.svg` stays live as this look's own badge silhouette, which is what keeps it distinct from `checkpoint_post`'s (`guard_standing.svg`). |
| Steam | `steam_grate.svg`, `steam.svg` and `steam_b.svg`: the vent's floor grate and the two frames of its cloud. **The grate is not the blow's picture**: a 32×32 ground decal, an iron grate seen from above, centred on the vent's point, which `InteriorEvents` lays at every vent for the whole section through `EventInstance.steam_grate()`, so the vent is on screen blowing or not. It is ground rather than something standing on it, since she walks over it between blows: a child of the building at the floor tiles' own `z_index`, after them in the tree, under the walls and under the y-sorted layer she and every blow stand in — not in that layer, where the basement's puddles are and where a decal is drawn over her feet whenever she stands on its northern half. Each blow is an instance drawing only the cloud through `_draw_simple()`: two 32×48 frames whose bottom-centre anchor is the grate's middle, so the column comes up out of the slots. The second frame is the plume billowing — the cloud lifted and spread, a fresh puff pushing up out of the grate under it — and the two alternate off `_idle_stepping(STEAM_BILLOW_PERIOD)`, a 0.7s cycle, for the instance's whole life, which is one blow of the vent. The blow's own body shadow falls on the grate while it blows, the same patch every solid row puts down under its body. |
| Loudspeaker mast | `mast.svg` (20×58, bottom-centre anchor): one standing picture, pole and horns, no heading to select — `EventInstance._draw_mast()` draws it through `_draw_simple()` the same way `mouse` and `steam` are. `mast_lamp.svg` (8×8, centre anchor) is a small overlay near the horn housing, tinted by `Sprites.draw_standing()`'s own `modulate` argument rather than baked as a second picture: `Palette.SIGNAL_AMBER` during the repeating telegraph, `Palette.SIGNAL_GREEN` while it speaks, undrawn when `silenced` — the player's 2026-09-23 amendment asking for a visible live/silenced and telegraph/speaking indicator. `sound_pulse.svg` (48×32, source anchor (24, 32)) is drawn above the lamp while the mast speaks — see "Sound lines" in `docs/EVENTS.md`, "The visual vocabulary". |
...
The [environment source review](evidence/svg-environment-2026-09-10/INVENTORY.md) lists the
interior, ground, roof, frontage and prop sources it reviewed in `sources.csv`, with native
canvases, anchors and alpha bounds. It includes tile repetition, facade overlays and chalk on
pavement.

**These reviews and the people matrix below record sources, not bindings.** Each gives a source's
canvas, anchor and alpha bounds as it was reviewed; which of them the game reads is this
catalogue's to say and nobody else's, and a source redrawn since its review carries its current
registration in the live tables above.
...
trim, gait and action source and its registration. Native/3×
...
Every file below is authored and none is read by the game, and no queued design would read one:
each is superseded by a picture that is live, or belongs to a view its caller can never reach.
None is on an atlas page except the last row's three, which are baked on the `events` page and
named by an `EventInstance` constant that nothing draws.

| Assets | Why nothing binds them |
|---|---|
| `art/crowd/car_end_{body,trim}.svg` (30×46) | The crowd car's foreshortened top-down end view, superseded by the standing `car_{front,back}_*` views the eight sectors draw. |
| `art/events/{police_car,fire_engine,unmarked_van,army_truck}_end.svg` | Each family's single end view, drawn for north and south headings alike, superseded by its `_front`/`_back` pair. |
| `art/events/riot_van_end.svg` (34×50, anchor (17, 50)) | The same picture `riot_van_front.svg` draws, which the night raid selects for that heading. |
| `art/events/gunman_{front,back,side,front_diagonal,back_diagonal}.svg` (30×26) | A firefight's two shooters face along the fixed axis between them, which `gunman.svg` already draws; `gunman_side.svg` is the same picture as `gunman.svg`. |
| `art/events/moving_van_{front,back,front_diagonal,back_diagonal}.svg` (34×50 ends, 56×50 diagonals) | The moving-van seal chooses between parallel to its street and across it, and `moving_van_vertical.svg` is its own authored across-the-street scene, ramp down and doors open. |
| `art/events/explosion_preview.svg` (40×40) | An explosion in the escape is off screen by definition and draws nothing — see `EventCatalogue._finale_explosion()`. |
| `art/interior/stair_rail_{e,w}.svg`, `stair_rail_level.svg` (32×32) and `stair_newel.svg` (16×40) | The stair tiles are a stairwell seen from the side and draw their own balustrade, so there is no run for a separate rail to be laid along. |
| `art/props/impact_crater_1x1.svg` (32×32, centre anchor (16, 16)) and `impact_crater_3x3.svg` (96×96, centre anchor (48, 48)) | One explosion row leaves one crater, and `impact_crater_2x2.svg` is the one it draws. |
| `art/events/cafe_sitter.svg`, `chatting_mother_talking.svg` and `van_victim.svg` | Each family's old single picture, superseded by its named views; none is a badge silhouette. |
...
| `poster_tear_a_mask.svg`, `poster_tear_a_overlay.svg` | Tear A, one of the three a sheet she pushes against shows (`PosterWalls._tear()` picks one by a hash of the run, the cell and the count of tears): the top and upper left of the sheet stay pasted, plus a scrap of the bottom-right corner. |
