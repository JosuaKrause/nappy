...
| `stairwell_segment_backdrop.svg`, `stairwell_shaft_cap_{top,bottom}.svg`, `stair_landing_vertical.svg` | Each floor landing with a flight below receives a repeatable 9×8-tile shaft bay, capped above the top door and below the lobby landing. Every full-flight start receives the same narrow 32×64 vertical landing platform: its east placement begins at the landing column and its west placement ends there, so it joins the corresponding mirrored stair side. The basement entry and every full flight use their existing tile-sized ground sources alone outside this review overlay. No broad flight deck, floor or turn landing overlay, foreground rail or rear rail is bound. The tile-sized flight and landing sources remain the ground and walking registration; no visual overlay changes collision, slope redirection or the plan. |
| `m158_stair_side_{upper,lower,continue}_{e,w}.svg` | `InteriorScene._add_flight_side()` binds the reviewed 32×32 sources in the structural layer alongside the vertical landing platform. Three complete upper/lower/continuation modules fill each full flight's five-tile height: every east module shifts one tile right and down, every west module shifts one tile left and down, and no extra end role is needed. Neither `InteriorTileSet` nor `InteriorMap` loads them, so collision, slope redirection, landings and the route stay unchanged. The native/3× source renders and two-module shaft-background review are in `docs/evidence/m158-stair-tile-prototype-2026-09-19/`. |
