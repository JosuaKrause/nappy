...
| M106 — Roofs, fronts and street trees | `tree_pit.svg` is a prepared, centre-anchored 32×32 ground part — see "City ground and buildings" above for the roof units and the fronts, now live. `assets/buildings/storefront_{a,b,c,d}_shuttered.svg` and `window_{tall,shuttered}_{dark,lit}.svg` stay unbound: the shuttered states are M105's, the city's own degradation, and the tall/shuttered window overlays are not named by this milestone. |
