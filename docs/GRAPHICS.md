...
| `art/buildings/fire_escape_a.svg`/`fire_escape_b.svg` | `src/city/building.gd` bolts one to a seeded share of `RESIDENTIAL` facades tall enough for it, as an overlay over the ground floor's two bottom rows — a composition change distinct from the storefront's, since an overlay draws after the wall rather than replacing one of its textures. |
