...
|---|---|
| M102 — The finale: out of the apartment, out of the city (interior) | `assets/events/explosion_preview.svg` is a 40×40 optional burst and stays prepared and unbound: an explosion in the escape is off screen by definition and draws nothing at all, so a picture for one would be a burst nobody is ever in a position to see — see `EventCatalogue._finale_explosion()`. `assets/interior/hallway_wall_window_flash.svg` and `assets/events/steam.svg` are now live — see "Interior (the escape scene)" and "Events and seal pictures" above. The rest of `assets/interior/` is bound by the escape scene itself. |
