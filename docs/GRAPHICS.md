...
| `art/buildings/window_boarded_sealed.svg` | `src/city/building.gd` draws this 32×32 overlay, in the windows' own registration and transparent elsewhere, over one upper-floor window cell of the one home-block building the door notch stands in front of — `Building.neighbor_window_col`, set once by `City.board_neighbor_window()` from day 11's morning on (`ResistanceHappenings.start_day()`), the third floor nearest the door or the topmost row on a front whose own height roll left it with fewer than three wall rows (`Building.neighbor_window_row()`; seed 4242's own home-block front rolls two). Three planks and the same notice as the door's, drawn over whichever window picture the cell already has so the front's own lintel and sill stay. |
