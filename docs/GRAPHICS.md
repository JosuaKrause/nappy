...
| Steam | `steam.svg` and `steam_b.svg`: two 32×48 frames standing on the ground they rise from, mirrored east/west like `mouse` and `skip` rather than switching pose, since a cloud has no front. The second is the same pipe with the plume billowing — the cloud lifted and spread, a fresh puff pushing out of the mouth under it — and the two alternate off `_idle_stepping(STEAM_BILLOW_PERIOD)`, a 0.7s cycle, for the instance's whole life, which is one blow of the vent. |
