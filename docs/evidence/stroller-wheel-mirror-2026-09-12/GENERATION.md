...
This is the donor-extraction stage. The
[final wheel arrangement](../stroller-southern-wheel-swap-2026-09-12/GENERATION.md) owns the
installed SE/SW pixels and reads a frozen output of this stage. Rebuilding either stage reads
its immutable inputs; never apply another exchange to installed wheels.
...
frozen source PNG. The donor is the frozen northeast diagonal PNG, whose wheel heading is
...
`11f278b6c7e584d7d4250365ad808c9142e9a65f576ef68ebad7115a50ad5699`. This stage's resulting
...
provides an optional handedness comparison; this stage's operation omits it.
...
Verify this stage's rebuilt derivative and the four unchanged stroller textures from the
repository root. The final wheel arrangement recipe verifies the installed SE derivative.
...
UV_CACHE_DIR=/tmp/nappy-uv-cache uv run python \
  ...
  --candidate /tmp/stroller-wheel-mirror-rebuild/pram_front_diagonal.png
```
