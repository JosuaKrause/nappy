...
Each folder of illustrated graphics or review evidence carries its own recipe — a `GENERATION.md`
or `README.md` with the retained inputs, the registration and the commands that rebuild it. Run
Python recipes through the repository's locked `uv` environment. Image generation can produce
different pixels on another call; extraction and assembly use the retained output.
...
| `archive/session-captures/2026-09-12/run-205023-seed3265820891-v0.8.2-870-g6ff43b3/` | Playtest 66's whole run: the log, seven asked-for stills of the map edges and twelve recorded bursts of stuck crowds, turning cars and their halos, and agents arriving at the plain edge. | [PLAYTEST-66.md](../playtests/PLAYTEST-66.md), `TODO.md` M119–M122 |
