...
end view for M103, the drawings the queue owes. The review added source art only; which of these
sources the game reads, and the current registration of any redrawn since, is
`docs/GRAPHICS.md`'s to say. No illustrated PNG derivative is part of this set.
...
consumer/use and owning work item. Paths are repository paths under `art/`.
...
mouse side and riot-van end sources.
...
for E. Other side sources face E. This describes authored geometry.
