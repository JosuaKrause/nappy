"""Register generated plaza paving without altering its illustrated material."""
...
def _repeat_sheet(tile: Image.Image, neighbors: dict[str, Image.Image]) -> Image.Image:
	...
	draw.text((2, 1), "plaza repeat", fill="white", font=font)
	for row in range(3):
		for column in range(6):
			sheet.alpha_composite(tile, (column * 32, 14 + row * 32))
	draw.text((2, 114), "shared ground bases", fill="white", font=font)
	for column, name in enumerate(("sidewalk_base", "plaza", "asphalt_base", "grass_base", "plaza", "sidewalk_base")):
		sheet.alpha_composite(tile if name == "plaza" else neighbors[name], (column * 32, 128))
	return sheet


def build(
	raw: Path, svg_render: Path | None, brightness_reference: Path | None,
	sidewalk_reference: Path | None, output_dir: Path, input_bundle: Path | None,
) -> None:
	_fresh(output_dir)
	if not raw.is_file():
	...
	if input_bundle is None and (svg_render is None or not svg_render.is_file()):
		raise ValueError("initial build requires --svg-render from render-plaza-source.gd")
	if brightness_reference is not None and not brightness_reference.is_file():
		raise ValueError(f"missing plaza brightness reference: {brightness_reference}")
	...
	shutil.copyfile(raw, output_dir / "raw/plaza-generated.png")
	inputs = output_dir / "frozen-inputs"
	if input_bundle is None:
		inputs.mkdir()
		shutil.copyfile(
			brightness_reference or RUNTIME / "plaza.png",
			inputs / "plaza_brightness_reference.png",
		)
	...
	registered.save(registered_dir / "plaza.png")
	...
	manifest = {
		...
		"raw": {"path": "raw/plaza-generated.png", "sha256": _sha256(output_dir / "raw/plaza-generated.png"), "size": list(raw_image.size)},
		"sources": {path.name: _sha256(path) for path in sorted(inputs.iterdir())},
		"registered": {"path": "registered/plaza.png", "sha256": _sha256(registered_dir / "plaza.png"), "size": list(registered.size), "opaque": registered.getchannel("A").getextrema() == (255, 255)},
		"mean_luma": {
			"plaza": _mean_luma(registered),
			"plaza_brightness_reference": _mean_luma(_load(inputs / "plaza_brightness_reference.png")),
...
def verify(bundle: Path, target: Path | None) -> None:
	...
	if target is not None and _sha256(target) != manifest["registered"]["sha256"]:
		raise ValueError("runtime plaza differs from registered tile")
...
def install(bundle: Path, target: Path, replace: bool) -> None:
	...
	verify(bundle, None)
	shutil.copyfile(bundle / "registered/plaza.png", target)
...
def main() -> None:
	...
	build_parser.add_argument("--brightness-reference", type=Path, help="preserve the old plaza brightness comparison input")
	build_parser.add_argument("--sidewalk-reference", type=Path, help="freeze the approved neighboring sidewalk material")
	...
	install_parser = commands.add_parser("install", help="replace the runtime plaza image from a verified bundle")
	...
	if arguments.command == "build":
		build(
			...
			arguments.sidewalk_reference.resolve() if arguments.sidewalk_reference else None,
