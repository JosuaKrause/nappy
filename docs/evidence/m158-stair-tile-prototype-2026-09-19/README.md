...
edge stays inked except their common top join. The gray section itself has no outline where it
touches the tile's left or bottom boundary, but keeps its internal diagonal edge. The continuation
tile is a 16×16 gray triangle at the top-right (top-left in the mirrored source): its legs span two
8px steps and bridge the lower side into the next shifted module without background art hiding the
join. The width is a silent, overturnable choice: 8px is the largest divisor of the 32px tile that
