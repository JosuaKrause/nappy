"""Assemble the father-only natural-leg refinement from frozen inputs."""
import argparse
import hashlib
...
ROOT = HERE.parents[4]
ACCEPTED = HERE.parent / "straight-contact-2026-09-19"
...
def input_record():
    paths = [
        Path(__file__),
        HERE / "side-donor-refined-raw.png",
        ACCEPTED / "side-donor-raw.png",
        ACCEPTED / "assemble.py",
        LOOPS / "assemble.py",
        P2,
    ]
    paths += sorted((ACCEPTED / "generated/rig").glob("father_*.png"))
...
def splice(source, diagonal):
    donor = rgba(HERE / "side-donor-refined-raw.png").crop(CROP)
    ...
    return result, {
        ...
        "composition": "new lower pair first; exact accepted B rows 0..27 last",
    }
...
def verify(output, old, p2):
    for view in VIEWS:
        for pose in ("a", "c", "b"):
            ...
            original = ACCEPTED / "generated/rig" / name
            ...
            if pose != "b" or view not in ("side", "front_diagonal"):
                ...
                upper = rgba(ACCEPTED / "generated/rig" / f"father_{view}_b.png")
...
def main():
    parser = argparse.ArgumentParser(
        description=__doc__, epilog="Example: %(prog)s --output-dir /tmp/father-leg-refinement"
    )
    ...
    for view in VIEWS:
        for pose in ("a", "c", "b"):
            ...
            if pose != "b" or view not in ("side", "front_diagonal"):
                shutil.copy2(ACCEPTED / "generated/rig" / name, target)
            else:
                source = rgba(ACCEPTED / "generated/rig" / f"father_{view}_b.png")
                picture, records[view] = splice(source, view == "front_diagonal")
                picture.save(target)
    old = module(LOOPS / "assemble.py", "old_father")
    p2 = module(P2, "p2_recipe")
    old.sheets(args.output_dir, p2)
    verify(args.output_dir, old, p2)
    record = {
        "status": "uninstalled early player-review refinement; static poses and offline animation only",
        "directions": p2.DIRECTIONS, "loop": old.LOOP, "frame_ms": [190] * 4,
        "registration": records, "input_manifest_sha256": digest(manifest),
        "protected": "accepted A/C plus front/back/back-diagonal B byte-identical; accepted side/front-diagonal B rows 0..27 byte-identical",
