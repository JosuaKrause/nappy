...
├── auto/
│   └── 143022-057-attempt1-lost_crying.png
└── asked/
    └── 143205-311-attempt1-asked.png
```
...
So a run writes PNGs into its own `auto/` folder, named `<HHMMSS>-<mmm>-attempt<N>-<what>.png` —
the wall-clock hour, minute, second and millisecond the picture was actually taken, not the day
clock: `--invincible` clamps the countdown to exactly zero once a day runs out rather than ending
it, which holds the elapsed day clock at the day's own length for as long as the day keeps running
afterward, so every picture taken past dusk that day would otherwise name itself identically; even
off that flag, two pictures inside one in-game second collide the same way. *(2026-09-11, playtest
56: "phot capture must use real time not game time otherwise at the end of the day all pictures
get overwritten".)* The attempt is named on every one, including the first, the same rule "A day
played twice" below states for the maps, and a name already on disk — two pictures in the same
millisecond — gets a serial (`-2`, `-3`, …) appended rather than overwriting the first. Three
rules:
...
`P` (or `F9`) writes `<HHMMSS>-<mmm>-attempt<N>-asked.png` — the same wall-clock naming `snapshot()`
uses, for the same reason — into the run's own `asked/` folder, kept apart from the heuristic's
`auto/` so a directory listing already says which of the two asked for a picture — and
