...
| `route` | `RouteRig` | **Under `--route` only: whether the rig's own day fits its clock.** A target reached — elapsed day time and distance walked, and for a mark or a task the moment the director records it done — a re-plan when the way ahead closed or a mark moved, a target given up on as unavailable, unreachable or physically stuck (a stuck one names the tile she stood on and what held her: an event row, a wall's tile type, a closure, a hold, or nothing solid, which is the crowd), and the day ending before a target was reached |
