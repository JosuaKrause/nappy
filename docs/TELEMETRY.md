...
| `texture` | `TextureResolver`, `TextureAtlas` | **When a picture was loaded or dropped, and what it cost** — a transfer PNG read from disk with the path and the milliseconds the read took, an atlas becoming ready with its group, how many pictures it holds, its pixel size, the milliseconds from the request and how much of that the worker thread took, and an atlas being released with how long it was drawn from. It is what says whether a picture arrived before it was drawn rather than during the frame that wanted it |
