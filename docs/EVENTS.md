...
| `dog_walker` | RECURRING | 1 | Mobile along the sidewalk at 32px/s — slower than walking, so the ordinary band rule applies. Intensity 27.5 on a tight radius, barking on a 3.5s pulse: it owns the pavement it is on, so walking straight through it is never the cheap option. **The measure is a real pass, not standing beside it**: walking past at `Tuning.WALK_SPEED` while it moves as it actually does nets +11.7/s awake at 0px of lateral offset against today's walking decay — `Tuning.WALL_WORTH_OF_COST` (35) is the ceiling `walk_through_cost()` (33.3) sits a 1.7-point margin under, and is why the pass restores about 80% of what it netted against the 3.5/s decay this row was first tuned beside rather than all of it. Deliberately given no `obstructs_radius` — a moving wall on a two-tile pavement pins the player against a building. |
...
| `homeless_yeller` | RECURRING | 1 | Intensity 20 over a 170px field, yelling on a 5s **pulse**, and **pacing** eight tiles of pavement (`EventDef.paces`). A fixed source on a fixed patch is a line you draw once; a man walking up and down it is a timing problem on top of a routing one. Mobile, so he has no body. **The measure is a real pass, not standing beside him**: walking past at `Tuning.WALK_SPEED` while he moves as he actually paces, in opposite directions, nets +9.6/s awake at 0px of lateral offset against today's walking decay — about 94% of the 3.5/s-decay pass this field once netted (+10.2/s). Two ceilings hold the field back from restoring the rest: `Tuning.WALL_WORTH_OF_COST` (35, `walk_through_cost()` sits a 1.4-point margin under) and the reach a beat needs to still turn up with nowhere to leave from at least once in a while (`EventScheduler._a_pacing_beat_walls_a_sidewalk`) — see "What an event actually costs" for the full table. **Friction or a wall depending on where his beat runs**: the way past him is never a lane and always a crossing — a beat that reaches a junction box is one she can leave at the zebra there while he is at the far end of it, and he stays on the route like any other timing problem; a beat truncated short of a junction by a closure or a calm zone leaves no way off his sidewalk, and that placement is a wall. His silhouette is his own — a long coat, a raised arm, a beard, one shape where a passer-by is two. |
...
| id | kind | from | Behaviour |
| --- | --- | --- | --- |
| `loose_dog` | RECURRING (`TOWARD_PLAYER`) | 1 | A dog whose owner has dropped the leash, sited on her own pavement when she gets close and running straight down it toward her. The counterpart to `dog_walker` and the reason both exist — that one is a **span** you decide whether to cross the street to avoid, this one is a **thing coming at you** that you cannot out-walk. 132px/s, so it earns a badge at the screen edge and pays the whole-radius telegraph. Not lethal, which is what separates it from `charging_dog`: this one is answered by getting out of the way, not by running. Intensity 39, up from 32 (Playtest 25, finding 6) for a bigger impact once a real meeting is priced against the fixed baseline rather than the barrier fields that used to pin it near the top of the meter regardless. **The measure is a real pass**: walking past at `Tuning.WALK_SPEED` while it charges, at 0px of lateral offset, nets +15.0/s awake against today's walking decay, over the +14.1/s the same pass netted against the 3.5/s decay this field was originally set beside — `TOWARD_PLAYER` never earns a `WALL` role, so nothing here trades against `Tuning.WALL_WORTH_OF_COST` the way `dog_walker`'s retuning does. 39/s rather than louder still is the running rule's own ceiling: past it, running past the dog costs less than walking past it, which nothing but `car_accident` is allowed to do. |
...
| `alley_robbery` * | +23.8 | — |
| `protest` | +27.6 | +77.6 |
| `dog_walker` | +33.3 | +42.6 |
| `homeless_yeller` | +33.6 | +57.9 |
| `busker` | +34.7 | +63.1 |
