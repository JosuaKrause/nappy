...
| id | kind | from | Behaviour |
| --- | --- | --- | --- |
| `fallen_tree` | SCRIPTED | day 1 | Placed fresh every morning as a **hard seal**: a trunk down kerb to kerb, root plate at one end and crown at the other. `obstructs_radius` 96, exactly half the 192px street, so `SealPlanner._hard_positions` places one body spanning it edge to edge — one continuous scene, not a repeated segment, drawn through a second, pre-rotated asset on an east-west street rather than through `_draw_spread`'s own axis relabelling (`EventInstance._wide_scene_texture`). |
| `car_accident` | SCRIPTED | day 1 | A **hard seal**: two cars locked together across the carriageway, glass between them and an onlooker on each pavement. Same single-body geometry as `fallen_tree`, including the pre-rotated sibling asset. |
...
| `burst_water_main` | SCRIPTED | day 1 | A **hard seal**: a crater with water across the asphalt and a municipal barrier at each kerb — the picture that explains why the road is out too. Same single-body geometry as `fallen_tree`, including the pre-rotated sibling asset. |
| `removal_lorry` | SCRIPTED | day 1 | A **soft seal**: a lorry at the kerb with its ramp down, the same body on each pavement. Drawn at `lorry.svg`'s own scale with a picture of its own (open doors, a ramp reaching the ground) — the catalogue's "no two rows share a look" rule means the literal `Look.LORRY` picture could not be reused. |
| `burnt_out_car` | SCRIPTED | day 4 | A **hard seal**, from act II onward: `burnt_shell`'s charred palette moved onto a vehicle body. Vehicle-scale `obstructs_radius`, so `SealPlanner._hard_positions` spaces four across a street the way it spaces two `barricade` copies — a pile-up rather than one wide scene, so it needs no rotated sibling. |
