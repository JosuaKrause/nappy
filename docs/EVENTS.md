...
| `construction` | RECURRING | 2 | `obstructs_radius` `EventCatalogue.SIDEWALK_SPREAD_MAX` (16px), the most a sidewalk-placed body may be — it fills the lane it stands on and forces a reroute onto the other one, and since a street is sidewalk\|road\|sidewalk, the far lane or the road is always still there, so it costs time and exposure, never the day. |
...
| `market_stall` | RECURRING | 1 | The second thing on day 1 that forces a crossing, and it exists because one obstacle repeated eighteen times is a rule rather than a decision. Louder, and on the other side of pleasant, than `cafe_tables`: a café you squeeze past is a nuisance, a market is a crowd. Not wider — it can land on `SIDEWALK` too, so its `obstructs_radius` is capped exactly the same. |
