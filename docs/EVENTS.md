...
| `moving_van` | SCRIPTED | day 1 | A **soft seal**: a lorry at the kerb with its ramp down, the same body on each pavement. Drawn at `lorry.svg`'s own scale with a picture of its own (open doors, a ramp reaching the ground) — the catalogue's "no two rows share a look" rule means the literal `Look.LORRY` picture could not be reused. |
