...
| `dog_walker` | RECURRING | 1 | Mobile along the sidewalk at 32px/s — slower than walking, so the ordinary band rule applies. Intensity 33 on a tight radius, barking on a 3.5s pulse: it owns the pavement it is on, so walking straight through it is never the cheap option. **The measure is a real pass, not standing beside it**: walking past at `Tuning.WALK_SPEED` while it moves as it actually does nets +16.1/s awake at 0px of lateral offset against today's walking decay, over the +14.7/s the same pass netted against the 3.5/s decay this row was first tuned beside — fully restored, with a margin. `walk_through_cost()` (42.7) sits under `Tuning.WALL_WORTH_OF_COST` (48) with a 5.3-point margin. Deliberately given no `obstructs_radius` — a moving wall on a two-tile pavement pins the player against a building. |
...
| `leaf_blower` | RECURRING | 1 | A man tidying a park, and a field with two parts: away from him it is the busker's exactly, so a park with a few of them spaced across it stays awake; inside `core_radius` — a pavement's own width — it is a wall. One of them closes one side of a street and never the street. `core_intensity` 35, up from 22.2 — the wall side of this row answers the same walking-decay rise `homeless_yeller` and `dog_walker` do, since it is what anchors `Tuning.WALL_WORTH_OF_COST`'s own line on the far side. `walk_through_cost()` (52.9) sits under nothing — it is a wall on purpose, with a margin over the 48 line this row's own retuning shares the credit for moving. |
