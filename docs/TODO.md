...
## M141 — The physics tick at thirty · asked for 2026-09-14

> "physics should be capped at 30fps at the least not 60"

[PLAYTEST-74](playtests/PLAYTEST-74.md). The phone reading with the crowd parked
(`DECISIONS.md`, M140, the phone reading) put the physics tick at 6 to 9 ms and showed it is
not the crowd's: `Crowd._physics_process` returning at once did not move it. The project
leaves the tick at the engine's default of sixty a second, so on a phone drawing 33 to 45
frames that tick runs once or twice a frame and costs twelve to eighteen of a frame's 22 to
30 ms. Halving the tick halves that, before anything in the tick itself is made cheaper. Thirty
is the number asked for; lower is a number the next phone reading can argue for, since
nothing below depends on thirty in particular.

**What runs on the tick, and what the halving does to each.** `Stroller._physics_process`
(her move, `move_and_slide`, the pram body, the walk phase, her camera follow — all
delta-scaled, so at thirty she covers the same ground in 3 px steps rather than 1.5);
`Baby._physics_process` (the meters, delta-scaled); `EventManager._physics_process`
(streaming, placing, summoning, detentions, hard fails — checks, run half as often);
`Crowd._physics_process` (signals delta-scaled, the rest checks); `ContactPoint`; and the
engine's own step over every body. Every test rig steps these by hand with its own `STEP`
constant and never reads the engine's rate, so the suites do not move. **The one thing that
changes visibly is her.** She and the camera's target move on the tick, and at thirty ticks
under a desktop drawing a hundred frames she would step every third frame while the camera's
own smoothing, applied at frame rate, glides — a walker jittering against a smooth view. The
engine's own answer is physics interpolation (`physics/common/physics_interpolation`), which
draws every physics-moved `Node2D` between its last two ticks; it is off by default and this
project has never turned it on. **It misreads anything moved in `_process`** — it would draw
such a node between two stale tick positions — and the crowd, the events and the resistance
director all move there, so those trees switch it off for themselves
(`physics_interpolation_mode = PHYSICS_INTERPOLATION_MODE_OFF` on `Crowd`, `EventManager`
and `ResistanceDirector`, inherited by everything under them). And a teleport under
interpolation is drawn as a one-tick slide from the old place to the new unless
`reset_physics_interpolation()` is called on the node after the move: `Stroller.teleport_to`
(a door release), `InteriorScene.teleport_to_door` (the escape's fade-and-teleport), the
day-start placement and the section restart, and the camera at `focus_camera_on` and at the
end of its ease back.

**The M100 item on the crowd's decision cadence** (`CrowdAgent` moves at frame rate; every
rule about that movement is applied at physics rate) quotes the ratio of movement steps to
right-of-way passes at the old tick; at thirty the desktop ratio doubles, and the item's own
recommendation — moving the agents to the physics tick — would put the crowd at thirty frames
a second, which is a different trade than it was. The entry's numbers are rewritten to the
new tick in the same PR, and its recommendation carries the note.

- [ ] **The tick at thirty, with her drawn between ticks.** `project.godot` gains
      `physics/common/physics_ticks_per_second=30` and `physics/common/physics_interpolation=true`.
      `Crowd`, `EventManager` and `ResistanceDirector` set
      `physics_interpolation_mode = Node.PHYSICS_INTERPOLATION_MODE_OFF` in `_ready()` or
      `_init()`, with a docstring saying why (moved in `_process`; interpolation would draw them
      between stale ticks). `reset_physics_interpolation()` follows every teleport of her —
      `Stroller.teleport_to`, `InteriorScene.teleport_to_door`, wherever `main._start_day()` or
      the section restart places her — and of the camera at `focus_camera_on` and where the
      ease back hands the camera back to its parented follow. A test in the presentation or
      pause suite pins `Engine.physics_ticks_per_second == 30` and
      `get_tree().physics_interpolation`, so the setting cannot drift out of `project.godot`
      unread. `docs/TELEMETRY.md`'s account of the `physics` column says the tick is thirty a
      second where it says what the column is; `docs/TODO.md`'s M100 cadence item is rewritten
      to the new ratio; grep the governed docs for `60 Hz`, `sixty` and `physics tick` for any
      other sentence that names the rate. Evidence: one desktop burst
      (`tools/shot.sh out.png 4 --seed 3265820891 --day 1 --walk 3s --press snapshot_burst 3`),
      to show she walks and the crowd walks; the smoothness judgement is the player's
      (`REVIEW.md`, in the same PR: does her walk read smooth on the desktop at a hundred
      frames, and does the phone's frame move — the four settings on seed 123 again).
