...
- **Walk the apartment and judge the remaining reference-based interior graphics.**
  ...
  fade-and-teleport read as a door or as a cut? The stair assembly and its traversal are open work
  under M158, the staircase follows the corrected tile grammar, after
  [PLAYTEST-81](playtests/PLAYTEST-81.md) rejected the first assembly. Records for the remaining
  interior are in `DECISIONS.md` under M112, the escape scene and interior graphics; what the
  finale lays on top of the building — its events, and the way out through the service door — is
  the escape walk in the list above.
