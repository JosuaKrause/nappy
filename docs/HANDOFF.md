...
- **The next work is the story's pacing, and it ends in a minor release**
  ...
  minor release"): M181, the resistance has a reason, and a task is one day, whose slice two —
  days 10 and 11, the once-only happenings, the neighbor, day 14's door at the power station —
  is open, and whose late days are then timed with the route rig (`--route`). It is decided in
  `TODO.md`
