...
**What it is now.** One number, `CityEdge.TUNNEL_DEPTH_TILES` (3), is the depth of the portal's
opening, and three things read it:

- **The road runs under the arch exactly that far and no further.** `_border_source` carries the
  carriageway three tiles north and paints mountain beyond; the bridge keeps the whole band,
  because a deck is in the open and a tunnel is inside the rock.
- **The fade is the opening.** The ramp is three steps, one per tile, alpha `1/3`, `2/3`, `1.0`,
  only as wide as the carriageway (`Tuning.carriageway_width()`, 64px), and the sprite's opening
  is transparent between the side walls so the darkening tiles show through it. Fully opaque at
  the ceiling, so a car has vanished before it reaches the top of the opening.
- **The portal is four tiles tall**, 192×128, with a stone band above a 96px opening, so the
  opening and the constant describe one hole.
...
corridor from the top of the portal to the far edge of the band, in the y-sorted layer where a car
north of the edge sorts behind it. The pixels are identical to the ground beneath, so the lid is
