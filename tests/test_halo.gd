extends RefCounted
## `ExcitementHalo.select_sources()`: which live events earn a place in the halo.
##
## The drawing itself — the shader, the colour, whether it reads as soft — is a screenshot's
## question and is out of this suite on purpose, the same line `docs/DECISIONS.md`'s testing
## policy draws for every other cue: "not tested, checked by eye: layout, colour, readability."
## What *is* testable, and would rot silently otherwise, is the selection this cue is built
## around — a day plans several hundred bodies and only a handful may ever earn a glow, or the
## cue marks everything and says nothing.
