extends RefCounted
## The crowd as traffic: population, contact and the street hierarchy's own physics.
##
## Split from a single `test_crowd.gd` under M125 -- "the test suite is slow again" -- by
## subject rather than trimmed: this suite is simulation-bound (real `Crowd.step` seconds, not
## assertion volume), so no seed or day count here buys back time by shrinking. This half keeps
## everything about the crowd as *traffic* -- how many are out, where they walk, what a contact
## or a strike costs, and the three-tier street hierarchy (ordinary / main / pedestrianised) that
## gives a route its shape. `test_crowd_closures.gd` keeps the other half: what the crowd does
## once a street is sealed, walled, bridged or tunnelled rather than plain open lattice.
##
...
## A car's picture is a standing elevation rather than a top-down one, so its own canvas has to be
## anchored at the point on it that is the car's actual ground contact — the drawn content's bottom
## edge on the strike box's own southernmost point, which is `CrowdAgent._car_body_anchor()`'s one
## rule for all five views. This pins the two cardinal cases against `Crowd._strike()`'s box, the
## shadow and the field, all of which are centred on the node and rotated onto the heading;
## `tests/test_car_views.gd` pins the same property for every sector, diagonals included, and pins
## that it does not jump at a sector boundary. Sampled the same way `Sprites.draw_standing` itself
## builds the rect, rather than by rasterising a frame.
func _test_the_car_picture_agrees_with_its_strike_box(t) -> void:
	...
	var front_drawn := Rect2(front_anchor - Vector2(front_extent.x * 0.5, front_extent.y),
			front_extent)
	t.check(is_equal_approx(front_drawn.end.y - front_margin, Tuning.CAR_STRIKE_HALF_LENGTH),
			("the front/back picture's own drawn bottom edge lands on the strike box's south edge "
			+ "(%.1f against %.1f)")
			% [front_drawn.end.y - front_margin, Tuning.CAR_STRIKE_HALF_LENGTH])
	...
	# **The side view is in the same family**, which it was not: a car pointed east has its box's
	...
	t.check(is_equal_approx(side_drawn.end.y - side_margin, Tuning.CAR_STRIKE_HALF_WIDTH),
			("the side picture's own drawn bottom edge lands on its box's south edge too (%.1f "
			+ "against %.1f)")
			% [side_drawn.end.y - side_margin, Tuning.CAR_STRIKE_HALF_WIDTH])
