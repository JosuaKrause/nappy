...
func run(t) -> void:
	...
	_test_only_walkers_receive_illustrated_presentation(t)
...
func _test_only_walkers_receive_illustrated_presentation(t) -> void:
	...
	for agent: CrowdAgent in _city.crowd.agents():
		if agent.kind == CrowdAgent.Kind.WALKER:
			walkers += 1
			t.check(agent.walker_visual != null and agent.has_node("ModularWalker"),
				"live walker has an aligned illustrated presentation child")
			t.check(agent.walker_visual.global_position == agent.global_position and
				agent.walker_visual.position == Vector2.ZERO,
				"walker presentation shares owner coordinates without an offset")
		else:
			cars += 1
