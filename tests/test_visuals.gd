...
func run(t) -> void:
	...
	_test_every_rig_transfer_has_a_native_svg_pair(t)
	TextureResolver.reset_for_tests(DevFlags.svg_requested())

func _test_every_rig_transfer_has_a_native_svg_pair(t) -> void:
	var directory := DirAccess.open("res://assets/illustrated/svg-transfer/rig")
	t.check(directory != null, "the rig transfer directory is available for the asset audit")
	if directory == null:
		return
	...
	while not filename.is_empty():
		if not directory.current_is_dir() and filename.ends_with(".png"):
			transfer_count += 1
			var transfer_path := "res://assets/illustrated/svg-transfer/rig/" + filename
			var source_path := "res://assets/rig/" + filename.trim_suffix(".png") + ".svg"
					...
					for y in source_image.get_height():
						for x in source_image.get_width():
							if not is_equal_approx(source_image.get_pixel(x, y).a,
									transfer_image.get_pixel(x, y).a):
								alpha_matches = false
					t.check(alpha_matches, "runtime PNG retains SVG alpha: %s" % filename)
		filename = directory.get_next()
	directory.list_dir_end()
	t.check(transfer_count > 0, "the rig transfer audit discovers runtime PNG assets")
