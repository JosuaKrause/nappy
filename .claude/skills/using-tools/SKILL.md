...
| `tools/record.sh` | Records any rig run through Godot's own movie writer, frame-locked and at the game's own resolution with its audio, then encodes it with ffmpeg into `build/records/` (gitignored); frames are deleted once it is encoded. `--player-view` and `--no-focus-pause` are added automatically. The rig has to quit on its own (a `--route` run does, on arrival) or its own `--after`/`--day-length` has to end it. `--out NAME.mp4` names the file. | To watch a rig run yourself rather than just reading its numbers — PLAYTEST-139, M214: "so I can create videos of those runs and review them." | cli-tools |
...
| `tools/trailer.sh` | Renders the trailer from the checked-in `tools/trailer/shots.json`: each shot through Godot's own movie writer, frame-locked and at the game's own resolution with its audio, then fades, joins and encodes them with ffmpeg into `build/trailer/trailer.mp4` (gitignored); frames are deleted as they are encoded. `--shot NAME` renders one shot alone, `--check NAME\|all` renders a shot twice and compares every frame's hash, `--list` prints the shot list without rendering. | To render or re-check the trailer, or one of its shots, after the shot list or a rig flag changes. | cli-tools |
