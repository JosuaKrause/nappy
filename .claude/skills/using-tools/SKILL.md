...
| `tools/land-prs.sh` | Lands a queue of already-authorized pull requests in order: merges each one directly when its `mergeStateStatus` already reads `CLEAN` (GitHub refuses to enable auto-merge on a PR that could merge right now), otherwise enables auto-merge, waits for each to merge (a PR merely behind `main` needs nothing, since the ruleset's checks are not strict; one that an earlier merge left conflicting stops the run with its auto-merge off, to be resolved with `tools/update-pr.sh` and reviewed under **pr-review** before it is landed again), then fast-forwards `main`, retires the branch with `tools/prune-merged.sh`, and points at `main`'s own CI run on the last merge as the check for two PRs that are wrong together. Stops, naming the PR and the reason, on a red check, a conflict, a PR closed unmerged, or a timeout. `--dry-run` reports the plan and each PR's state without touching anything. | To land several already-authorized PRs one after another without polling and merging each by hand. | committing |
...
| `tools/synthesize-sfx.py` | Rebuilds the original Copper lark A/B sound-effects audition: deterministic WAVs, the listening page, provenance manifest and portable ZIP. It writes files and never plays them. | To reproduce or deliberately revise the standalone grounded-versus-stylized sound comparison. | sound-effects |
