...
| `tools/resolve-decisions-top.sh` | Resolves `docs/DECISIONS.md`'s one recurring merge-conflict shape — both sides inserting a new `## …` section directly under `# Decisions`, from an empty base — writing ours above theirs with one blank line between, then `git add`s it only once `git diff --check` passes. Refuses and changes nothing for any other shape. | Mid-merge (`git merge --no-ff --no-commit origin/main` already run), when `docs/DECISIONS.md` conflicts. | merging-main |
