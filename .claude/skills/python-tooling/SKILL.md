...
pull request. In order: `ruff check`, `ruff format --check`, `mypy` (strict), then the two
`unittest` files. `tools/pycheck.sh --fix` lets ruff rewrite imports and formatting first.
