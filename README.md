...
| `--skip events\|crowd\|shadows` (comma-separated, any order) | Turn off one or more of the desktop's own per-frame draw probes — `events` empties every `EventInstance._draw`, `crowd` every `CrowdAgent._draw`, `shadows` empties `BuildingShadows._draw_chunk` — so their frame cost can be read on a device that has no probes of its own. Honoured only while the readout is on (`--debug` or a release web build's own `?debug=1&skip=events,crowd`); nothing else about the frame moves, and the readout's own `skip` line names what is off |
