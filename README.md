...
| `--debug` | Turn the developer readout on, and only the readout, in an exported release build (also reachable as a release web build's own `?debug=1`); no other dev flag or debug layer is reachable this way |
