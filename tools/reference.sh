#!/usr/bin/env bash
# Bring real-world reference photos and videos into docs/reference/, or an approved
# illustration-style reference into docs/style-references/ with --style.
...
# when a photo belongs in the repo at all and which of the two folders it belongs in.
#
...
usage() {
	cat <<'EOF'
usage: tools/reference.sh [--help|-h] [--style] [--force] <file-or-directory>...

Brings real-world reference photos and videos into docs/reference/, shrunk to fit inside
1280x720 with every scrap of metadata dropped. --style places the result in
docs/style-references/ instead, for an approved illustration-style reference rather than a
real-world photo. --force overwrites a file already there.

  tools/reference.sh <file-or-directory>...
  tools/reference.sh --style <file-or-directory>...
