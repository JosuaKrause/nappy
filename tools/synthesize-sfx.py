...
RECIPE_NOTES: Final = {
    ...
    "footsteps-pass-3-grounded.wav": (
        "The byte-preserved pass-3 grounded footsteps: two heel-and-toe contacts with restrained "
        "surface texture and no low thump."
    ),
    "footsteps-subtle-grounded.wav": (
        "Two quiet sole brushes with short, compressed midrange contacts and a faint toe release; "
        "the impact body is reduced and the level sits below pass 3."
    ),
    "stroller-wheels-pass-3-grounded.wav": (
        "The byte-preserved pass-3 grounded stroller wheels: dry wheel contacts, a rotation pulse "
        "and paired mechanism rattles."
    ),
    "stroller-wheels-quieter-grounded.wav": (
        "Very quiet dry wheel contacts with a faint rotation pulse and restrained single mechanism "
        "ticks; the level sits below the subtle steps."
    ),
    "comparison.wav": (
        "The selected auditions concatenated A then B within each pair, with 0.45 "
...
def _finish(values: Signal) -> Signal:
    return _finish_at_rms(values, TARGET_RMS_DBFS)


def _finish_at_rms(values: Signal, target_rms_dbfs: float) -> Signal:
    centered = _prepare(values)
    peak = max(abs(value) for value in centered)
...
def footsteps_subtle_grounded(seed: int) -> Signal:
    count = _frames(1.15)
    rng = _rng(seed, "footsteps-subtle-grounded")
    sole = _lowpass(_noise(count, rng), 2_700.0)
    scuff = _lowpass(_noise(count, rng), 1_200.0)
    values = [0.0] * count
    for start, weight in ((0.14, 1.0), (0.69, 0.86)):
        for index in range(round(start * SAMPLE_RATE), min(count, round((start + 0.13) * SAMPLE_RATE))):
            tau = index / SAMPLE_RATE - start
            contact_phase = 2.0 * math.pi * (255.0 * tau - 135.0 * tau * tau)
            contact = math.sin(contact_phase) * math.exp(-82.0 * tau)
            brush = sole[index] * math.exp(-118.0 * tau)
            drag = scuff[index] * _smooth_gate(tau, 0.014, 0.125, 0.018)
            values[index] += weight * (0.075 * contact + 0.11 * brush + 0.065 * drag)

        toe_start = start + 0.052
        for index in range(round(toe_start * SAMPLE_RATE), min(count, round((toe_start + 0.055) * SAMPLE_RATE))):
            tau = index / SAMPLE_RATE - toe_start
            toe = math.sin(2.0 * math.pi * 345.0 * tau) * math.exp(-92.0 * tau)
            values[index] += weight * (0.025 * toe + 0.018 * sole[index] * math.exp(-135.0 * tau))
    compressed = _compress_peaks(values, 0.22, 5.0)
    return _finish_at_rms(compressed, SUBTLE_STEP_RMS_DBFS)
...
def stroller_wheels_quieter_grounded(seed: int) -> Signal:
    count = _frames(2.2)
    rng = _rng(seed, "stroller-wheels-quieter-grounded")
    contact = _lowpass(_noise(count, rng), 1_850.0)
    values = [0.0] * count

    for index in range(count):
        t = index / SAMPLE_RATE
        rotation = max(0.0, math.sin(2.0 * math.pi * 4.35 * t)) ** 10
        values[index] = 0.011 * rotation * math.sin(2.0 * math.pi * 96.0 * t)
    ...
    while start < 2.11:
        ...
        start += 0.113 + rng.uniform(-0.004, 0.004)
    for contact_index, start in enumerate(rolling_starts):
        weight = 0.74 + 0.10 * (contact_index % 3)
        for index in range(round(start * SAMPLE_RATE), min(count, round((start + 0.04) * SAMPLE_RATE))):
            tau = index / SAMPLE_RATE - start
            tire = contact[index] * math.exp(-125.0 * tau)
            bump = math.sin(2.0 * math.pi * 148.0 * tau) * math.exp(-98.0 * tau)
            values[index] += weight * (0.085 * tire + 0.09 * bump)

    for start in (0.26, 0.62, 0.98, 1.35, 1.72, 2.03):
        click_start = start + rng.uniform(-0.002, 0.002)
        for index in range(round(click_start * SAMPLE_RATE), min(count, round((click_start + 0.022) * SAMPLE_RATE))):
            tau = index / SAMPLE_RATE - click_start
            tick = math.sin(2.0 * math.pi * 760.0 * tau) * math.exp(-165.0 * tau)
            values[index] += 0.035 * tick
    return _finish_at_rms(_compress_peaks(values, 0.26, 4.0), QUIET_WHEEL_RMS_DBFS)
...
SUBTLE_RECIPES: Final[dict[str, Recipe]] = {
    "footsteps-pass-3-grounded.wav": footsteps_revised_grounded,
    "footsteps-subtle-grounded.wav": footsteps_subtle_grounded,
    "stroller-wheels-pass-3-grounded.wav": stroller_wheels_revised_grounded,
    "stroller-wheels-quieter-grounded.wav": stroller_wheels_quieter_grounded,
}


def _configuration(selection: str) -> tuple[tuple[Pair, ...], dict[str, Recipe]]:
    if selection == "subtle-revision":
        return SUBTLE_PAIRS, SUBTLE_RECIPES
...
def _readme(seed: int, selection: str, pairs: Sequence[Pair]) -> str:
    ...
    if selection == "subtle-revision":
        comparison_description = (
            "Open `index.html` for the focused controls. A is the byte-preserved pass-3 grounded "
            "take and B is the quieter pass-4 proposal."
        )
        level_description = (
            "The pass-3 WAVs are byte-identical to the submitted pass 3 and retain its -22.5 dBFS "
            "RMS target. The new footsteps target -31.0 dBFS RMS with a softer compressed contact; "
            "the new stroller wheels target -38.0 dBFS RMS with restrained mechanism ticks. Both "
            "retain the 0.70 peak ceiling."
        )
    elif selection == "grounded-revision":
        comparison_description = (
...
def _review_page(selection: str, pairs: Sequence[Pair]) -> str:
    ...
    if selection == "subtle-revision":
        introduction = (
            "Two grounded level revisions. Compare each byte-preserved pass-3 take with a much "
            "subtler proposal; the new wheels sit below the new steps."
        )
        level_summary = "pass 3 preserved · new steps -31 dBFS RMS · new wheels -38 dBFS RMS"
    elif selection == "grounded-revision":
        introduction = (
        ...
        level_summary = "OLD is byte-identical pass 1 · revised uses a -22.5 dBFS RMS target and 0.70 peak ceiling"
    else:
        introduction = (
            "Four original procedural effects, each in a tactile grounded treatment and a clearly synthetic treatment."
        )
        level_summary = "common RMS target with a 0.70 peak ceiling"
...
def generate(output: Path, seed: int, selection: str) -> None:
    ...
    manifest = {
        ...
        "mix_target_rms_dbfs": (
            {
                "pass_3_preserved": TARGET_RMS_DBFS,
                "subtle_footsteps": SUBTLE_STEP_RMS_DBFS,
                "quieter_wheels": QUIET_WHEEL_RMS_DBFS,
            }
            if selection == "subtle-revision"
            else TARGET_RMS_DBFS
        ),
        "level_strategy": (
            "Pass-3 files preserve their bytes; new footsteps and wheels use separate lower RMS targets, "
            "with wheels lower."
            if selection == "subtle-revision"
            else (
