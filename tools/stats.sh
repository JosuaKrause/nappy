...
#
#   tools/stats.sh          # playtest runs only (run-*.log)
#   tools/stats.sh --rigs   # rig runs only (rig-*.log) -- check.sh, shot.sh, --walk/--flee/--press
...
# A "day" is one `home` or `lost` line; a "run" is one log file, and one sitting can hold several
# days if the process was never restarted, so the two counts are not the same thing.
set -uo pipefail
shopt -s nullglob
...
case "$mode" in
	playtest) print_group "playtest runs (run-*.log)" "run" ;;
	rigs)     print_group "rig runs (rig-*.log)" "rig" ;;
	all)
		print_group "playtest runs (run-*.log)" "run"
		print_group "rig runs (rig-*.log)" "rig"
		;;
esac
