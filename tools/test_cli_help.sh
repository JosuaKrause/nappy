...
# anything that is not --serial/--plan/--record-costs/--help/-h straight to the test scene as
# either a suite-name substring or a flag the scene itself reads off OS.get_cmdline_user_args()
# (`--svg` is a real, documented example) -- there is no fixed list to validate that free-text
# surface against, so only its --help path is checked there. --record-costs is its own recognised
# flag with its own shape to get wrong, though, and validates before the import pass that would
# otherwise touch the Godot stub -- so its malformed case is asserted below alongside the rest.
#
...
# test.sh's own --record-costs gets the same treatment as the rest of tools/: a malformed shape
# is rejected -- usage, non-zero, no launch -- before the import pass that would otherwise touch
# the Godot stub.
